...
def render_pdf_preview(filepath: Path, max_w: int, max_h: int) -> ImageTk.PhotoImage | None:
...
def render_image_preview(filepath: Path, max_w: int, max_h: int) -> ImageTk.PhotoImage | None:
    if not HAS_PIL:
        return None
    try:
...
def render_docx_preview(filepath: Path, max_w: int, max_h: int) -> ImageTk.PhotoImage | None:
...
class SidebarItem(tk.Frame):
    def __init__(self, parent, index: int, name: str, callback):
        super().__init__(parent, bg="#161820", height=36)
        self.index = index
        self.callback = callback
        self.active = False

        self.num = tk.Label(self, text=f"{index + 1}.", bg="#161820", fg="#3e4257", font=("Segoe UI", 10, "bold"))
        self.num.pack(side=tk.LEFT, padx=(10,0))

        self.lbl = tk.Label(self, text=name, bg="#161820", fg="#9098b5", font=("Segoe UI", 11), anchor="w")
        self.lbl.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,10))
    ...
    def set_active(self, active: bool):
        self.active = active
        if active:
            self.config(bg="#252840")
            self.num.config(fg="#6c8eff")
            self.lbl.config(fg="#d0d5f0")
        else:
            self.config(bg="#161820")
            self.num.config(fg="#3e4257")
            self.lbl.config(fg="#9098b5")
...
class ZoomWindow(tk.Toplevel):
    def __init__(self, photo: ImageTk.PhotoImage, parent=None):
        ...
        img.thumbnail((max_w, max_h))
        scaled_photo = ImageTk.PhotoImage(img)

        lbl = tk.Label(self, image=scaled_photo)
        lbl.image = scaled_photo  # keep reference
        lbl.pack()
...
class FileRenamer(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.canvas = tk.Canvas(self.scroll_frame, bg="#161820", highlightthickness=0)
        scrollbar = ttk.Scrollbar(self.scroll_frame, orient=tk.VERTICAL, command=self.canvas.yview)
        self.scrollable_frame = tk.Frame(self.canvas, bg="#161820")
    ...
    def _build_sidebar(self):
        ...
        for i, f in enumerate(self.files):
            item = SidebarItem(self.scrollable_frame, i, f.name, self._jump_to)
    ...
    def _update_sidebar_active(self, index: int):
        ...
        if 0 <= index < len(self.sidebar_items):
            self.after(50, lambda: self.canvas.yview_moveto(index * 36 / self.canvas.bbox("all")[3]))
    ...
    def rename_file(self):
        ...
        if self.current_index < len(self.sidebar_items):
            self.sidebar_items[self.current_index].lbl.config(text=dest.name)
    ...
    def delete_file(self):
        if not self.files or self.current_index >= len(self.files):
            return
        filepath = self.files[self.current_index]
        reply = messagebox.askyesno("Confirm Delete", f"Permanently delete\n{filepath.name}?")
        if not reply:
            return
        try:
            filepath.unlink()
    ...
    def _remove_current_and_advance(self):
        ...
        for i in range(idx, len(self.sidebar_items)):
            ...
            self.sidebar_items[i].num.config(text=f"{i + 1}.")
    ...
    def _zoom_preview(self, event):
        if self._full_photo:
            ZoomWindow(self._full_photo, self)
